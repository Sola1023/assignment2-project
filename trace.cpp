...
bool Triangle::intersect(const Ray &r, double t0, double t1, HitRecord &hr) const {
  ...
  // If it's a miss just return false.
	SlVector3 v0 = b - a;
	SlVector3 v1 = c - a;
	SlVector3 n = cross(v0, v1);
	double ndr = dot(n, r.d);
	if (fabs(ndr) < 1e-4) return false;
	double t = (dot(n, a) + dot(n, r.e)) / ndr;
	if (t < 0)return false;
...
bool Sphere::intersect(const Ray &r, double t0, double t1, HitRecord &hr) const {
  ...
  //
	SlVector3 v = r.e - c;
	const double bb = 2 * dot(v, r.d);
	const double cc = dot(v, v) - rad * rad;
  ...
  // return true
	const double t11 = (-bb - sqrt(delta)) / 2;
	const double t2 = (-bb + sqrt(delta)) / 2;
	hr.t = (t11 < t2) ? t11 : t2;
	SlVector3 x = r.e + hr.t * r.d;
	hr.p = x;
	hr.n = hr.p - c;
	normalize(hr.n);
  // <<>>
